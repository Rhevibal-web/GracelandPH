  ...
  $filecheckstat = $check !== false ? true : false;
   ...
   $file_stat = checkImage($_FILES["itemimagefile"], $target_dir, $target_filename);
    ...
    if(!mysqli_stmt_prepare($stmt_chk, $sql_check)){
        header("location: control_dashboard.php?error=3"); //statement failed
        exit();
    }
    ...
    if(!empty($arr)){
        header("location: control_dashboard.php?error=1&itemname={$itemname}"); //item exist
        exit();
    }
    else{
        ...
        header("location: control_dashboard.php?error=2"); //insert failed
        exit();
        }
        ...
        if(!$file_err_count){
            ...
            if (move_uploaded_file($image_temp_file, $target_dir."/".$target_filename) ) {
                ...
                header("location: control_dashboard.php?error=99"); //file upload failed
                exit();
              }
            
             header("location: control_dashboard.php?error=0&itemname={$itemname}"); 
        }
